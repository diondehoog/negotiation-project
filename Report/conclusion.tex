...
In the tests we have seen that our acceptance strategy in combination with phase 3 
almost always leads to acceptance, which is apparently too lenient compared to other agents.
Furthermore, when optimizing phase 2 this resulted in parameters that were not a good
fit for our agent at all, sometimes resulting in very early acceptance. We have decided 
in final tests to disable concessions in phase 2 to prevent this from happening.

We conclude that in automated negotation, starting out with a simple agent and 
continueally improving this agent is a better strategy to develop agents. 
We clearly started out with a too complex agent, before optimizing any parameters.
However, we think that by seperating phases in a bidding strategy can lead to 
improvements than just using a single bidding strategy. By adding parameters
to each individual phase, we can explore the space of strategies even more,
just like the BOA model which makes it easier to search in the space of strategies.
...
Because our agent used quite complex behaviour, we tried to use as much functions
...
before accepting.
\item Conversions between bidDetails, bid and bidPoint classes can be hard. Especially 
converting bidPoints back to bid or bidDetails seemed impossible, we had to search through
part of the bidspace to find back the bids corresponding to bidPoints.
\item Built in functions of the opponentSpace do not work for all default included 
opponent models. For example, we were not able to construct bidspaces for most
models except the hardheaded frequency model. 
